...
- **Estimated DPS** — with no exact match, a team whose first two slots (or, failing that, first and third) match a projection team still gets a figure, prefixed `~` and marked **estimate** in amber: the last of the matching teams, i.e. the lowest-ranked, so it reads as a floor rather than a promise. It appears as soon as slots 1 and 2 are filled.
- **Slots swapped** — a flat **5%** comes off when only the last slot differs from the projection's team (including when it is still empty), and **25%** when the second slot is the one that differs and only the first and third match. Either way a `−5%` / `−25%` chip says so. The 1+2 pairing is tried first; 1+3 is the looser fallback.
